...
class SurgicalSimulator(SurgicalSimulatorBase):
    def __init__(self, env_type, env_params,id=None,demo=None):
        ...
        self.previous_contact_result = False
        self.contact_info_change = False
    ...
    def load_trajectory(self):
        ...
        if self.id == 8:
            ...
            current_psm_position = self.env._get_robot_state(idx=0)[0:3]
            print('current psm position is: ', current_psm_position, 'psm contact is: ', self.contact_result)
    ...
    def before_simulation_step(self):
        try:
            print('recoed force:',self.mr.body.measured_cf())
            self.force.append(np.array(self.mr.body.measured_cf()))
        except Exception as e:
            print(str(e))
        if (self.id == 8 or self.id == 6) and (not self.has_load_policy or self.contact_info_change):
            obs = self.env._get_obs()
        ...
        if self.env.ACTION_SIZE != 3 and self.env.ACTION_SIZE != 1:
            if self.demo:
                ...
                if self.id in self.full_dof_list:
                    ...
                    if self.contact_result != self.previous_contact_result:
                        self.contact_info_change = True
                        self.previous_contact_result = self.contact_result
                        print('state changed')

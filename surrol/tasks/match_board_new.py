...
class MatchBoard(PsmEnv):
    ...
    def _env_setup(self):
        ...
        match_board = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'match_board_new/match_board.urdf'),
        ...
        if self.object in ['A', 'B', 'C']:
            fn = 'match_board_new/'+chr(ord(self.object)-ord('A')+ord('a'))+'.urdf'
        else:
            fn = 'match_board_new/'+str(self.object)+'.urdf'
